...
# Authentic v1 container generated by the pre-v2 implementation at d3f24c3.
LEGACY_V1_FIXTURE = Path(__file__).parent / "fixtures" / "wallet-v1-pending-rotation.dw"


def _copy_legacy_v1_fixture(target: Path) -> bytes:
...
def test_explicitly_migrate_v1_container_with_pending_key_to_v2(tmp_path: Path):
    source = tmp_path / "wallet.dw"
    original = _copy_legacy_v1_fixture(source)

    assert json.loads(original)["version"] == 1
    with pytest.raises(UnsupportedFormat):
        Wallet.open(source, PASSWORD)
    imported_path = tmp_path / "imported.dw"
    with pytest.raises(UnsupportedFormat):
        Wallet.import_container(imported_path, original, PASSWORD)
    assert not imported_path.exists()

    migrated = Wallet.migrate_container(source, PASSWORD)
    try:
        assert json.loads(source.read_bytes())["version"] == 2
        assert source.read_bytes() != original
        assert migrated.public_key.hex() == (
            "9f8544ce97a2bae6d53c788ca472ca29a58f156ab7cf481f8a6512af86f3c68e"
        )
        assert migrated.prepare_signing_key_rotation().hex() == (
            "abd600e02eec04f20e14c73c8c1744a201f5745fa9b00aa85b1c48119d2e4cd9"
        )
        signer = migrated.create_signer()
        assert signer.public_key == migrated.public_key
        signer.invalidate()
    finally:
        migrated.lock()

    imported = Wallet.import_container(imported_path, source.read_bytes(), PASSWORD)
    try:
        assert imported.public_key == bytes.fromhex(
            "9f8544ce97a2bae6d53c788ca472ca29a58f156ab7cf481f8a6512af86f3c68e"
        )
        assert imported.prepare_signing_key_rotation() == bytes.fromhex(
            "abd600e02eec04f20e14c73c8c1744a201f5745fa9b00aa85b1c48119d2e4cd9"
        )
    finally:
        imported.lock()

    reopened = Wallet.open(source, PASSWORD)
    try:
        assert reopened.public_key.hex() == (
            "9f8544ce97a2bae6d53c788ca472ca29a58f156ab7cf481f8a6512af86f3c68e"
        )
        assert reopened.prepare_signing_key_rotation().hex() == (
            "abd600e02eec04f20e14c73c8c1744a201f5745fa9b00aa85b1c48119d2e4cd9"
        )
    finally:
        reopened.lock()
...
def test_migration_rejects_non_predecessor_versions_without_rewriting(
    tmp_path: Path,
):
    source = tmp_path / "wallet.dw"
    wallet = Wallet.create(source, PASSWORD, PASSWORD, {"revision": 1})
    wallet.lock()
    ...
    for version in (0, 2, 3):
        ...
        with pytest.raises(UnsupportedFormat):
            Wallet.migrate_container(source, PASSWORD)
        assert source.read_bytes() == raw


def test_v1_migration_wrong_password_preserves_source(tmp_path: Path):
    source = tmp_path / "wallet.dw"
    before = _copy_legacy_v1_fixture(source)

    with pytest.raises(UnlockFailed):
        Wallet.migrate_container(source, "definitely wrong passphrase")

    assert source.read_bytes() == before
    assert json.loads(source.read_bytes())["version"] == 1


def test_tampered_v1_payload_is_rejected_without_rewriting_source(tmp_path: Path):
    source = tmp_path / "wallet.dw"
    _copy_legacy_v1_fixture(source)
    ...
    tampered = json.dumps(original, separators=(",", ":")).encode()
...
def test_v1_migration_directory_sync_failure_restores_source(
    ...
    monkeypatch.setattr(container_module, "_fsync_directory", fail_first_sync)
...
def test_v1_migration_pre_replace_failure_preserves_source(
    tmp_path: Path, monkeypatch
):
    source = tmp_path / "wallet.dw"
    before = _copy_legacy_v1_fixture(source)

    def fail_stage(_path: Path, _data: bytes, *, replace: bool) -> tuple[int, int]:
        raise OSError("injected pre-replacement stage failure")

    monkeypatch.setattr(container_module, "_stage_file", fail_stage)
    with pytest.raises(StorageFailure):
        Wallet.migrate_container(source, PASSWORD)
...
def test_v1_migration_reports_unknown_if_rollback_fails(
    tmp_path: Path, monkeypatch
):
    source = tmp_path / "wallet.dw"
    before = _copy_legacy_v1_fixture(source)
    ...
    monkeypatch.setattr(container_module, "_stage_file", fail_rollback_stage)
    monkeypatch.setattr(container_module, "_fsync_directory", fail_after_replace)
    with pytest.raises(StorageOutcomeUnknown):
        Wallet.migrate_container(source, PASSWORD)
    assert json.loads(source.read_bytes())["version"] == 2
    assert source.read_bytes() != before
    assert list(tmp_path.glob(".decent-wallet-*")) == []
    current = Wallet.open(source, PASSWORD)
    current.lock()


def test_wallet_locks_after_unverified_storage_rollback(
    tmp_path: Path, monkeypatch
):
    path = tmp_path / "wallet.dw"
    wallet = Wallet.create(path, PASSWORD, PASSWORD, {"revision": 1})
    original_stage = container_module._stage_file
    original_sync = container_module._fsync_directory
    stage_calls = 0
    sync_calls = 0
    ...
    def fail_rollback_stage(
        stage_path: Path, data: bytes, *, replace: bool
    ) -> tuple[int, int]:
        nonlocal stage_calls
        stage_calls += 1
        if stage_calls == 2:
            raise OSError("injected rollback failure")
        return original_stage(stage_path, data, replace=replace)
    ...
    current = Wallet.open(path, PASSWORD)
    current.lock()
...
def test_directory_sync_failure_restores_previous_container(tmp_path: Path, monkeypatch):
    ...
    def fail_once(_directory):
        nonlocal sync_calls
        sync_calls += 1
        if sync_calls == 1:
            raise OSError("simulated directory sync failure")
        original_sync(_directory)

    monkeypatch.setattr(container_module, "_fsync_directory", fail_once)
    wallet = Wallet.open(path, PASSWORD)
    with pytest.raises(StorageFailure):
        wallet.update_payload({"revision": 2})
    assert sync_calls == 2
