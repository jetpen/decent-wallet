...
Status: accepted MVP design with completed wallet storage, signing, Identity adapter, multisignature, Python-core owner-key rotation publication/confirmation/finalization, explicit Python-core v1-to-v2 container migration, and a normative v2 wire-format vector; native platform adapters and cross-platform conformance remain incomplete
...
- a normative v2 container wire-format specification and public synthetic known-answer vector, verified by the Python test suite; native implementations have not yet consumed the vector;
